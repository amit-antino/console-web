<div class="modal fade" id="editconditionModalMaster{{___encrypt($count)}}" tabindex="-1" role="dialog" aria-labelledby="add_conditionLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="POST" role="edit_form_condition{{___encrypt($count)}}" action="{{url('experiment/experiment/setpoint-master-condition-list')}}">
                ...
                <div class="modal-body">
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <select data-target="#setpoint-master-condition-value{{$count}}" data-count="{{$count}}" data-method="POST" data-type="condition" data-request="ajax-append-fields" data-url="{{url('experiment/experiment/range-value-field')}}" class="form-control" id="criteria" name="criteria">
                                <option value="">Select Criteria</option>
                                @if(!empty($criteria))
                                @foreach($criteria as $criterias)
                                <option @if($master_condition['criteria']==$criterias->id) selected @endif value="{{___encrypt($criterias->id)}}">{{$criterias->name}}</option>
                                @endforeach
                                @endif
                            </select>
                        </div>
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <div class="col-md-6">
                                            ...
                                            <div id="setpoint-master-condition-value{{$count}}">
                                                @if(!empty($master_condition['max_value']))
                                                <input type="number" min="0.0000000001" data-request="isnumeric" value="{{$master_condition['max_value']}}" class="form-control" id="max_value" name="max_value" placeholder="Enter Value" />
                                                @endif
                                            </div>
                                        </div>
                                        <div class="col-md-6">
