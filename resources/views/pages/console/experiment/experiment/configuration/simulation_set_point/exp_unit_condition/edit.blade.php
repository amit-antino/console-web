<div class="modal fade" id="editUnitconditionModal{{$count}}" tabindex="-1" role="dialog" aria-labelledby="add_conditionLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <form method="POST" action="{{url('experiment/experiment/setpoint-unit-condition-list?process_experiment_id='.$unit_condition['experiment_id'])}}" role="edit_output_exp_unit_condition{{$count}}">
                        ...
                        <div class="form-group col-md-12">
                            <div class="form-row">
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <select data-target="#setpoint-exp-unit-condition-value{{$count}}" data-count="{{$count}}" data-method="POST" data-type="condition" data-request="ajax-append-fields" data-url="{{url('experiment/experiment/range-value-field')}}" class="form-control" id="criteria" name="criteria">
                                        <option value="">Select Criteria</option>
                                        @if(!empty($criteria))
                                        @foreach($criteria as $criterias)
                                        <option @if($unit_condition['criteria']==$criterias->id) selected @endif value="{{___encrypt($criterias->id)}}">{{$criterias->name}}</option>
                                        @endforeach
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="form-group col-md-12">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <div id="setpoint-exp-unit-condition-value{{$count}}">
                                            @if(!empty($unit_condition['max_value']))
                                            <input type="number" min="0.0000000001" data-request="isnumeric" value="{{$unit_condition['max_value']}}" class="form-control" id="max_value" name="max_value" placeholder="Enter Value" />
                                            @endif
                                        </div>
