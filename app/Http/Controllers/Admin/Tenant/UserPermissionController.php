...
class UserPermissionController extends Controller
{
    ...
    {
        $user_permissions = UserPermission::where('tenant_id', ___decrypt($tenant_id))->get();
        $perm = [];
        ...
        return view('pages.admin.tenant.user_permission.index', compact('perm', 'tenant_id'));
    }
    ...
    public function create($tenant_id)
    {
        ...
        return view('pages.admin.tenant.user_permission.create', compact('data', 'tenant_id'));
    }
    ...
    public function store(Request $request, $tenant_id)
    {
        ...
        if ($validator->fails()) {
            ...
            $this->status = true;
            $this->modal = true;
            $this->redirect = url('admin/tenant/' . $tenant_id . '/user_permission');
    ...
    public function edit($tenant_id, $id)
    {
        ...
        return view('pages.admin.tenant.user_permission.edit', compact('data', 'tenant_id', 'permissions', 'user_perms'));
    }
    ...
    public function group_user_list(Request $request, $tenant_id)
    {
        ...
        if (empty($request->parameters)) {
            ...
            if (!empty($admin_users)) {
                foreach ($admin_users as $key => $users) {
                    ...
                    if (empty($check_user)) {
                        $admin_user[$key]['id'] = $users['id'];
                        $admin_user[$key]['name'] = $users['first_name'] . ' ' . $users['last_name'];
                    }
                }
            }
            //  $data['user'] = $user_data;
        ...
        return response()->json([
            'status' => true,
            'html' => view('pages.admin.tenant.user_permission.user_list', ['user_arr' => $user_arr, 'tenant_id' => $tenant_id])->render()
        ]);
    }
}
