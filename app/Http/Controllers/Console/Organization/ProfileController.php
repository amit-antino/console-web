...
class ProfileController extends Controller
{
    public function index(Request $request)
    {
        ...
        $countries = Country::where([['currency','!=','']])->get();
        return view('pages.console.tenant.profile', compact('user_info', 'countries', 'timezones', 'preference'));
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            if ($request->role == 'basic_info') {
                ...
                $user_settings['is_two_factor_enable'] = !empty($request->is_two_factor_enable) ? $request->is_two_factor_enable : 'off';
                $users['settings'] = $user_settings;
            } else {
            ...
            $this->status = true;
            $this->modal = true;
            $this->redirect = url('organization/profile');
            $this->message = "Profile Updated Successfully!";
        }
        return $this->populateresponse();
    }

    public function default_preference(Request $request)
    {
        $users = User::find(Auth::user()->id);
        // $users['language'] = 'english';
        // $users['timezone'] = 10;
        // $users['currency_type'] = 101;
        // $users['date_format'] = 'dd-mm-yyyy';
        $users['updated_by'] = Auth::user()->id;
        $users['updated_at'] = now();
        $users->save();
        $this->status = true;
