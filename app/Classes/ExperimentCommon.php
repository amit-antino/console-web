...
class ExperimentCommon
{
    ...
    public  function experiment_list($redis_update = '')
    {
        ...
        if (empty($cachedExperiment) || !empty($redis_update)) {
            if (Auth::user()->role == 'console') {
                $process_experiments = ProcessExperiment::where('created_by', Auth::user()->id)->where('tenant_id', session()->get('tenant_id'))->with('experiment_category', 'get_project')->orderBy('id', 'desc')->get();
            } else {
                $process_experiments = ProcessExperiment::where('tenant_id', session()->get('tenant_id'))->with('experiment_category', 'get_project')->orderBy('id', 'desc')->get();
            }
            $process_exp_list = [];
            $classifications = [];
            foreach ($process_experiments as $process_experiment) {
                ...
                $variations = [];
                if (!empty($varcount)) {
                    foreach ($varcount as $var) {
                        $variations[] = $var['id'];
                    }
                }

                $simInputcount = SimulateInput::whereIn('variation_id', $variations)->where('experiment_id', $process_experiment->id)->get();
                if (Auth::user()->role != 'admin') {
                    ////////---------IMPORTANT-----------/////////
                    #uncomment these condition when project required else it will be comentted
            ...
            Redis::del('process_exp_list' . session()->get('tenant_id') . '_sim_' . Auth::user()->id);
            Redis::set('process_exp_list' . session()->get('tenant_id') . '_sim_' . Auth::user()->id, json_encode($process_exp_list), 'EX', 120);
        } else {
